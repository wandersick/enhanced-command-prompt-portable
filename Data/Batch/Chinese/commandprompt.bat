...
ECHO       │  
ECHO       │  CommandPromptPortable.exe
ECHO       │  
ECHO       ├─Data
ECHO       │  └─Batch
...
ECHO       │          listc.bat
ECHO       ├─Exe
ECHO       │  ├─Microsoft (example)
ECHO       │  ├─UnxUtils (example)
ECHO       │  │  ├─...  
ECHO       │  │  └─...  
ECHO       │  │      └─...
ECHO.
...
echo # 現在的時間是：%date% %time:~0,-3%

REM would be more convenient to start here than root

cd Exe
echo.
echo # 預設指令：視窗預設指令 (ref: help), 歡迎訊息 (commandprompt), etc.
echo.
...
echo.
echo # 如欲新增指令，請放於 "Exe"；查詢子資料夾指令：輸入 dirc /? 或 listc /?
echo.
